...
BUCKET_LABELS = ("00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15", "16")
...
def read_total_ops(line, file_id):
    total = int(line[line.find("(") + 1: line.find(" total)")])
...
def usage():
    ...
    "    e.g. actout.txt"
    print
    " -t analysis slice interval in seconds"
    print
    "    default: 3600"
    print
    ...
    "    default: 3"
    print
    " -x (show extra information for each slice)"
    print
    "    default: not set"
...
def main(arg_log, arg_slice, arg_num_buckets, arg_every_nth, arg_extra):
    ...
    if arg_slice < 1:
        print
        "slice must be more than 0"
        sys.exit(-1)
    if arg_num_buckets < 1:
        print
        "num_buckets must be more than 0"
        sys.exit(-1)
    if arg_every_nth < 1:
        print
        "every_nth must be more than 0"
        sys.exit(-1)
    ...
    if not line:
        print
        "can't find any output data"
        sys.exit(-1)
    if line.split(" ")[2] == "version":
        version = line.split(" ")[3]
    print
    "data is act version " + version
    ...
    try:
        ...
        print
        "can't determine reporting interval"
        sys.exit(-1)
    ...
    if not line:
        print
        "can't find histograms' scale, assuming milliseconds"
        file_id.seek(0, 0)
    ...
    if slice_time != arg_slice:
        print
        "analyzing time slices of " + str(slice_time) + " seconds"
    ...
    print
    prefix_pad + GAP_PAD + " trans " + justify_pad + GAP_PAD + " device"
    print
    prefix_pad + GAP_PAD + scale_label + justify_pad + GAP_PAD + \
    scale_label
    print
    labels_prefix + GAP_PAD + threshold_labels + GAP_PAD + \
    threshold_labels
    ...
    if which_slice:
        ...
        print
        "could not find " + str(slice_time) + " seconds of data"
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "l:t:n:e:x", ["log=", "slice=", "num_buckets=", "every_nth=", "extra"])
