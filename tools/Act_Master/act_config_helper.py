    ...
    def do_createconfig(self, line):
        ...
        try:
            ...
            while (not response.isdigit()):
                response = input('Enter the number of devices you want to create config for: ')
                if response == '':
                    continue
            ...
            for device in range(1, no_of_devices + 1):
                response = ''
                while response == '':
                    response = input('Enter device name #' + str(device) + ' (e.g. /dev/sdb or /dev/sdb1): ')
                    if not response == '':
            ...
            # Test duration:
            td = input('Change test duration default of ' + str(test_duration_sec / 3600) + ' hours? (y/N): ')
            if 'y' == td or 'Y' == td:
                response = 'x'
                while (not response.isdigit()):
                    response = input('Enter the test duration in hours: ')
                    if response == '':
                        continue
                tds = int(response)
            ...
            response = input('Use non-standard configuration? (y/N): ')
            if 'y' == response or 'Y' == response:

                # Read & write rates:
                rwops = input('Configure read/write ops per second? (y/N): ')
                if 'y' == rwops or 'Y' == rwops:
                    response = 'x'
                    while (not response.isdigit()):
                        response = input('Enter read ops per second for each device: ')
                        if response == '':
                            continue
                    read_reqs_per_sec = int(response) * no_of_devices
                    response = 'x'
                    while (not response.isdigit()):
                        response = input('Enter write ops per second for each device: ')
                        if response == '':
                            continue
                ...
                # Record size:
                recsz = input('Change record-bytes default of ' + str(record_bytes) + '? (y/N): ')
                if 'y' == recsz or 'Y' == recsz:
                    response = 'x'
                    while (not response.isdigit()):
                        response = input('Enter record-bytes value: ')
                        if response == '':
                            continue
                    record_bytes = response

                # Queues & threads:
                numq = input('Change num-queues default of ' + str(num_queues) + '? (y/N): ')
                if 'y' == numq or 'Y' == numq:
                    response = 'x'
                    while (not response.isdigit()):
                        response = input('Enter num-queues value: ')
                        if response == '':
                            continue
                    num_queues = response
                tpq = input('Change threads-per-queue default of ' + str(threads_per_queue) + '? (y/N): ')
                if 'y' == tpq or 'Y' == tpq:
                    response = 'x'
                    while (not response.isdigit()):
                        response = input('Enter threads-per-queue value: ')
                        if response == '':
                            continue
            ...
            if use_standard:
                ...
                while (not response.isdigit()):
                    ...
                    response = input('Enter the load factor (e.g. enter 1 for 1x test): ')
                    if response == '':
                        continue
            ...
            response = ''
            while response == '':
                response = input('Do you want to save this config to a file? (y/N): ')
                if 'y' == response or 'Y' == response:
                    if device_load > 0:
                        ...
                        filename_load_tag = str(read_reqs_per_sec / no_of_devices) + 'r' + str(
                            write_reqs_per_sec / no_of_devices) + 'w'
                    actfile = 'actconfig_' + filename_load_tag + '_' + str(no_of_devices) + 'd.txt'
                    try:
                        act_file_fd = open(actfile, "w")
                        act_file_fd.write('##########\n')
                        act_file_fd.write('act config file for testing ' + str(
                            no_of_devices) + ' device(s) at ' + load_tag + ' load\n')
                        act_file_fd.write('##########\n\n')
