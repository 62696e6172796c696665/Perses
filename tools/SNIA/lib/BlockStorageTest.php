...
abstract class BlockStorageTest {
  ...
  public static function getRunOptions() {
    ...
    // implicit nopurge
    if (isset($options['nosecureerase']) && $options['nosecureerase'] && isset($options['notrim']) && $options['notrim'] && isset($options['nozerofill']) && $options['nozerofill']&& isset($options['noformat']) && $options['noformat']) $options['nopurge'] = TRUE;
   ...
   * arguments --nosecureerase, --notrim, --nozerofill and --noformat. Returns TRUE on 
  ...
  public final function purge() {
    ...
    $noformat = isset($this->options['noformat']) && $this->options['noformat'];
    ...
    if (!$nopurge) {
      foreach($this->options['target'] as $target) {
        ...
        print_msg(sprintf('Attempting to purge %srotational target %s with --nosecureerase=%d; --notrim=%d; --nozerofill=%d; --noformat=%d', $rotational ? '' : 'non-', $target, $nosecureerase ? '1' : '0', $notrim ? '1' : '0', $nozerofill ? '1' : '0', $noformat ? '1' : '0'), $this->verbose, __FILE__, __LINE__);
        ...
        // 2017.09.25 start
        // Add by Beishi Chen
        // Add format function to do purge
        // next try Format
        // if (!$purged && !$rotational && !$noformat) {
        if (!$purged && !$noformat) {
          print_msg(sprintf('Attempting format for target %s', $target), $this->verbose, __FILE__, __LINE__);
          $cmd = sprintf('nvme format %s -l 0 >/dev/null 2>&1; echo $?', $target);
          $ecode = trim(exec($cmd));
          if ($ecode > 0) {
              print_msg(sprintf('NVME Format not supported or failed for target %s', $target), $this->verbose, __FILE__, __LINE__);
              print_msg(sprintf('Maybe you forget to install nvme-cli ^_^'), $this->verbose, __FILE__, __LINE__);
          }
          else {
            print_msg(sprintf('NVME Format successful for target %s', $target), $this->verbose, __FILE__, __LINE__);
            $this->purgeMethods[$target] = 'format';
            $purged = TRUE;
          }
        }
        else if (!$purged) print_msg(sprintf('NVME Format not attempted for target %s because %s', $target, $noformat ? '--noformat argument was specified' : 'device is rotational'), $this->verbose, __FILE__, __LINE__);
