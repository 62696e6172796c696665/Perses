...
function hmshowPopup(e, txt, stick) {
    ...
    if (ns4) {
        ...
        if ((mx + offsxy + bodyl + tooltip.width) > bodyw) {
            mx = bodyw - offsxy - bodyl - tooltip.width;
            if (mx < 0) mx = 0;
        }
        ...
        if (tooltip.offsetWidth) if ((mx + offsxy + bodyl + tooltip.offsetWidth) > bodyw) {
            mx = bodyw - offsxy - bodyl - tooltip.offsetWidth;
            if (mx < 0) mx = 0;
        }
    ...
    with (tooltip) {
        ns4 ? visibility = "show" : style.visibility = "visible"
    }
